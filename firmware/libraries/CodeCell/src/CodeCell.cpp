...
// Patched: expose the raw game rotation vector quaternion (w, x, y, z) so
// downstream code can avoid the Euler decomposition (gimbal lock, frame
// entanglement). Requires MOTION_ROTATION_NO_MAG to be enabled in Init().
void CodeCell::Motion_RotationNoMagVectorRead(float &w, float &x, float &y, float &z) {
  if (((_msense & MOTION_ROTATION_NO_MAG) == MOTION_ROTATION_NO_MAG) || (_wakeup_flag == 1)) {
    w = _motion_data[4];
    x = _motion_data[5];
    y = _motion_data[6];
