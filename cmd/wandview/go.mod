module github.com/anthonyrego/wand/cmd/wandview

go 1.24.1

require (
	github.com/Zyko0/go-sdl3 v0.0.0-20251211154533-f3a75a84a029
	github.com/anthonyrego/construct v0.0.0
	github.com/anthonyrego/wand v0.0.0
	github.com/go-gl/mathgl v1.2.0
)

require (
	github.com/Zyko0/purego-gen v0.0.0-20250727121216-3bcd331a1e0c // indirect
	github.com/ebitengine/purego v0.9.0-alpha.2.0.20250124174847-29f0104e3c2b // indirect
	golang.org/x/sys v0.7.0 // indirect
)

replace (
	github.com/anthonyrego/construct => ../../../construct
	github.com/anthonyrego/wand => ../..
)
